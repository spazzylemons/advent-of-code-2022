...
(defun part-1 (monkeys)
  (loop until (monkey-value (gethash "root" monkeys))
        do (maphash #'(lambda (name monkey)
                        (when (not (monkey-value monkey))
                          (let ((value-a (monkey-value (monkey-lhs monkey)))
                                (value-b (monkey-value (monkey-rhs monkey))))
                            (when (and value-a value-b)
                              (setf (monkey-value monkey) (apply (monkey-op monkey) (list value-a value-b)))))))
             monkeys))
  (monkey-value (gethash "root" monkeys)))
...
; (defun format-equation (monkey)
;   (cond ((equal (monkey-name monkey) "humn")
;          "x")
;         ((monkey-value monkey)
;          (format nil "~a" (monkey-value monkey)))
;         (t
;          (format nil "(~a ~a ~a)" (format-equation (monkey-lhs monkey))
;                                   (format-op (monkey-op monkey))
;                                   (format-equation (monkey-rhs monkey))))))

(defun part-2-simplify (monkey)
  (if (and (not (monkey-value monkey)) (not (equal (monkey-name monkey) "humn")))
    (let* ((monkey-a (part-2-simplify (monkey-lhs monkey)))
           (monkey-b (part-2-simplify (monkey-rhs monkey)))
...
(defun part-2 (monkey)
  (let ((lhs (monkey-lhs monkey))
    ...
    (loop
      (setf lhs (part-2-simplify lhs))
      (setf rhs (part-2-simplify rhs))
...
(let ((monkeys (parse-input "input")))
  (format t "part 1: ~a~%" (part-1 monkeys)))
